...
do
    ...
    echo "<p>省代码和市代码: 不能为空<p> 开具来源: 不能为空,取值集合:YY=营业,WT=网厅,ZT=掌厅,SG=手工开票,ZZ=自助终端,SF=第三方平台,PD=PDA,TY=天翼生活APP,YX=易销售<p>开票端代码: 不能为空,各省自行定义<p>下载来源:  空表示未下载或者未上传下载渠道参数,取值集合 YJ,YC,YD,UJ,UC,UD,UZ,UW,XW<hr>" > ./${sh%%_*}字段上传明细.html
     ...
     #mysql -uroot -prootoo -H voh -e "select PROVINCE_CODE as 省代码,city_code as 市代码,kpd as 开票端,kjly 开具来源,xzqd as 下载渠道,xzrq as 下载日期,fpqqlsh as 发票请求流水号,kprq 开票日期,fpdm as 发票代码,fphm as 发票号码  from fpmx1015  where   XHDWDM in ($line)  " >> ./${sh%%_*}字段上传明细.html
    mysql -uroot -prootoo -H voh < tozhangc.sql   >> ./${sh%%_*}字段上传明细.html
