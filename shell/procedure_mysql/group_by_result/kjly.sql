...
-- repeate loop
DECLARE My_Cursor CURSOR FOR  SELECT  kjly  FROM fpmx1018 where XHDWDM like 'shuih%' group by kjly ; -- 定义游标并输入结果集
  ...
  myLoop: LOOP -- 开始循环体,myLoop为自定义循环名,结束循环时用到
      FETCH My_Cursor into fpcl_lv; -- 将游标当前读取行的数据顺序赋予自定义变量12
          IF done THEN -- 判断是否继续循环
			...
			-- set @sqll=concat('select count(*)/','@a, count(*),',fpcl,', xhdwmc,fpqqlsh,fphm,fpdm from fpmx1018 where ',fpcl,'=','''',fpcl_lv,'''', ' and XHDWDM like ','''','shuih%'  ,''''    ,' limit ',lnum);
